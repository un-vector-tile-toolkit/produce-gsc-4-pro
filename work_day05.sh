...
for f in produce-gsc-osm-46/mbtiles/osm_tile_day05/*.mbtiles; do tile-join --no-tile-size-limit -f -o large_tiles/un/osm/tile_day05/`basename ${f}` produce-gsc-un-46/mbtiles/un_tile/`basename ${f}` produce-gsc-osm-46/mbtiles/osm_tile_day05/`basename ${f}`; date; echo  `basename ${f}`; ls -alh large_tiles/unosm/tile_day05/`basename ${f}` ;done
