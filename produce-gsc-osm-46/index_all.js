...
let conversionTilelist01 = config.get('day01Tilelist') 
let conversionTilelist02 = config.get('day02Tilelist') 
let conversionTilelist03 = config.get('day03Tilelist') 
let conversionTilelist04 = config.get('day04Tilelist') 
let conversionTilelist05 = config.get('day05Tilelist') 
let conversionTilelist06 = config.get('day06Tilelist') 
let conversionTilelist07 = config.get('day07Tilelist') 
...
winston.configure({
  ...
  transports: [ 
    new DailyRotateFile({
      filename: `${logDir}/produce-gsc-osm-%DATE%.log`,
...
const main = async () => {
  winston.info(`${iso()}: Whole tile production started.`)
  await getScores()
  queueTasks()
  queue.on('drain', () => {
    shutdown()
